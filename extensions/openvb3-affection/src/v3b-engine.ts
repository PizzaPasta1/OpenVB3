...
export type PresenceState = "ACTIVE" | "BRB" | "AWAY" | "SLEEP";
...
export async function loadOrInitState(workspace: string): Promise<AffectionStateV3b> {
  ...
  try {
    ...
    const parsedPresenceState = (parsed as any).presence?.state;
    const isSleeping = parsedPresenceState === "SLEEP";
    ...
    //
    // If user is sleeping, freeze these "background" drifts until they return.
    // (Still reset daily counters like today.affGain.)
    const didDailyRollover = !isSameDay;
    const didFreezeDailyDrift = didDailyRollover && isSleeping;
    const beforeDaily = { reliabilityTrust, irritation };
    if (didDailyRollover && !didFreezeDailyDrift) {
    ...
    if (didDailyRollover) {
      // Persist the daily reset the first time we notice a new day.
      // If sleeping, we *skip* the drift and do not log a daily-decay delta.
      await saveState(workspace, state);
      if (!didFreezeDailyDrift) {
        await appendAudit(workspace, {
          ts: isoNow(),
          action: "touch",
          note: "daily-decay",
          deltas: {
            reliabilityTrust: state.reliabilityTrust - beforeDaily.reliabilityTrust,
            irritation: state.irritation - beforeDaily.irritation,
          },
        });
      } else {
        await appendAudit(workspace, {
